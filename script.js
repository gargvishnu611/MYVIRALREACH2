...
let currentTrack=0,currentTime=0,isPlaying=false,isMuted=false,audioCtx=null,gainNode=null,osc=null,timer=null;
...
function setupAudio(){
 ...
 gainNode=audioCtx.createGain();gainNode.gain.value=isMuted?0:.045;gainNode.connect(audioCtx.destination);
}
function pulseSound(){
 ...
 g.gain.setValueAtTime(0,now);g.gain.linearRampToValueAtTime(isMuted?0:.035,now+.015);g.gain.exponentialRampToValueAtTime(.001,now+.28);
...
function startPlayback(){
 ...
 timer=setInterval(()=>{
   currentTime+=.1;
   if(currentTime>=30){currentTime=0;nextTrack();}
   updateUI();
 },100);
 pulseSound();
}
...
function loadTrack(index){
 currentTrack=(index+tracks.length)%tracks.length;currentTime=0;
 const t=tracks[currentTrack];[playerTitle,stageTitle].forEach(e=>e.textContent=t.title);[playerArtist,stageArtist].forEach(e=>e.textContent=t.artist);durationEl.textContent='0:30';
 document.querySelectorAll('.track-card').forEach((card,i)=>card.classList.toggle('selected',i===currentTrack));updateUI();
}
...
playButton.addEventListener('click',()=>isPlaying?stopPlayback():startPlayback());$('nextButton').addEventListener('click',nextTrack);$('prevButton').addEventListener('click',prevTrack);
progress.addEventListener('input',e=>{currentTime=(Number(e.target.value)/100)*30;updateUI()});
$('muteButton').addEventListener('click',()=>{isMuted=!isMuted;$('muteButton').textContent=isMuted?'MUTE':'VOL';if(gainNode)gainNode.gain.value=isMuted?0:.045});
document.querySelectorAll('.track-card').forEach(card=>card.addEventListener('click',()=>{loadTrack(Number(card.dataset.index));startPlayback()}));
$('shuffleButton').addEventListener('click',()=>{let next=Math.floor(Math.random()*tracks.length);while(next===currentTrack)next=Math.floor(Math.random()*tracks.length);loadTrack(next);startPlayback()});
...
$('lyricsButton').addEventListener('click',()=>{$('lyrics').scrollIntoView({behavior:'smooth'})});$('artistButton').addEventListener('click',()=>document.querySelector('.visual-stack').scrollIntoView({behavior:'smooth',block:'center'}));

let pointerX=0,pointerY=0,targetX=0,targetY=0;
if(stage){stage.addEventListener('pointermove',e=>{const r=stage.getBoundingClientRect();targetX=((e.clientX-r.left)/r.width-.5)*24;targetY=((e.clientY-r.top)/r.height-.5)*-18});stage.addEventListener('pointerleave',()=>{targetX=targetY=0});}
function animatePointer(){pointerX+=(targetX-pointerX)*.08;pointerY+=(targetY-pointerY)*.08;if(albumWrap)albumWrap.style.transform=`rotateX(${pointerY}deg) rotateY(${pointerX}deg)`;requestAnimationFrame(animatePointer)}animatePointer();

const searchModal=$('searchModal'),searchInput=$('searchInput'),searchResults=$('searchResults');
function renderResults(q=''){const items=tracks.filter(t=>(t.title+' '+t.artist).toLowerCase().includes(q.toLowerCase()));searchResults.innerHTML=items.map((t,i)=>`<div class="result" data-track="${tracks.indexOf(t)}"><strong>${t.title}</strong><small>${t.artist}</small></div>`).join('')||'<div class="result"><small>No matching demo tracks.</small></div>';searchResults.querySelectorAll('[data-track]').forEach(r=>r.addEventListener('click',()=>{loadTrack(Number(r.dataset.track));startPlayback();searchModal.classList.remove('open');searchModal.setAttribute('aria-hidden','true')}))}
$('searchButton').addEventListener('click',()=>{searchModal.classList.add('open');searchModal.setAttribute('aria-hidden','false');setTimeout(()=>searchInput.focus(),50);renderResults()});$('closeSearch').addEventListener('click',()=>{searchModal.classList.remove('open');searchModal.setAttribute('aria-hidden','true')});searchModal.addEventListener('click',e=>{if(e.target===searchModal){searchModal.classList.remove('open');searchModal.setAttribute('aria-hidden','true')}});searchInput.addEventListener('input',e=>renderResults(e.target.value));
